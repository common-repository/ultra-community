...
abstract class MchBasePlugin
{
	...
	public function initializePlugin()
	{
		...
		\add_action('shutdown', array(MchWpTaskScheduler::getInstance(), 'scheduleRegisteredTasks'));
	}
...
//		\function_exists('wp_create_nonce') || require_once( ABSPATH . WPINC . '/pluggable.php' );
		...
		\add_action('init', function(){
			...
			$pluginSlug = MchBasePlugin::getPluginSlug();

			$locale = \apply_filters('plugin_locale', get_locale(), $pluginSlug);

			\load_textdomain($pluginSlug, trailingslashit( \WP_LANG_DIR ) . $pluginSlug . \DIRECTORY_SEPARATOR . $pluginSlug . '-' . $locale . '.mo' );

			\load_plugin_textdomain($pluginSlug, false, $pluginSlug . \DIRECTORY_SEPARATOR . 'languages' . \DIRECTORY_SEPARATOR );
