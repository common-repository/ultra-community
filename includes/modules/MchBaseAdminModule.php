...
{
	CONST FILTER_FIELD_SETTINGS_TABLE_ROW_OUTPUT = 'mch-field-settings-table-row-output';

	//private $arrDefaultOptionsValues = array();
	private $arrRegisteredMessages   = array();

	public abstract function getDefaultOptions();
	public abstract function validateModuleSettingsFields($arrSettingOptions);
...
//		return $this->arrDefaultOptionsValues;
		...
		$arrDefaultOptionsValues = array();

		foreach((array)$this->getDefaultOptions() as $optionName => $arrOptionInfo)
		{
			$arrDefaultOptionsValues[$optionName] = isset($arrOptionInfo['Value']) ? $arrOptionInfo['Value'] : null;
		}

		return $arrDefaultOptionsValues;
	...
	public function getFormattedMessagesForDisplay()
	{
		...
		$htmlCode = '<div class = "mch-settings-message" style = "{holder-style}"><h3 style = "margin-bottom: 5px; padding: 10px;  font-size:15px; line-height: 1.4">{message}</h3></div>';
